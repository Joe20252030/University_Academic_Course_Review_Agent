...
| Area | Change |
|---|---|
| **Auto-updater** | The app checks for new GitHub releases silently 4 s after launch. A non-blocking dialog offers Update Now (download + open/launch installer), Remind Me Later, and Skip This Version (persisted). macOS opens the `.dmg` in Finder; Windows launches the installer detached and exits the app. Pre-releases are included in the check. Any network failure is handled silently. |
| **Auto-updater: pre-release support** | Switched from the `/releases/latest` endpoint (which silently excludes pre-releases and returns 404 if all releases are pre-releases) to the `/releases` list endpoint. All releases including pre-releases are checked; the highest-versioned candidate with a matching platform asset is offered. |
| **Auto-updater: macOS SSL fix** | PyInstaller-frozen apps on macOS cannot access the system certificate store, causing `CERTIFICATE_VERIFY_FAILED` on every HTTPS request. Fixed with `_make_ssl_context()` backed by `certifi`'s bundled CA bundle, applied to every `urlopen` call in the updater. |
...
| **API workspace cleanup symlink guard** | `_cleanup_expired_api_workspaces` now applies two independent symlink checks: an initial check when a workspace is first considered for cleanup, and a TOCTOU re-check immediately before `shutil.rmtree(ws)`. The original code had neither. Mirrors the defence-in-depth already in `delete_session()`. |
...
| **Updater dialog close-during-download** | Two cases where closing the update dialog via `×` could still silently launch the installer (or quit the app on Windows). Fixed with two independent `winfo_exists()` guards: one in `_on_download_done` (dialog closed while downloading) and one inside `_apply_now` itself (dialog closed during the 500 ms / 1 s settle delay). Both paths delete the downloaded file and clear the pending path. |
